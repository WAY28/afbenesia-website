...
const partners = [
  ...
  { name: "PT Maju Jaya", abbr: "PT Maju\nJaya" },
  { name: "Dinas Perindustrian Jombang", abbr: "Dinas\nJombang" },
  { name: "Startup X", abbr: "Startup\nX" },
  ...
  return (
    <div className="font-sans">
      ...
      <section className="relative min-h-[92vh] flex items-center overflow-hidden">
        ...
        <div className="absolute top-24 right-24 w-8 h-8 bg-accent opacity-40 pointer-events-none z-10" />
        ...
        <div className="relative z-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 w-full">
          <div className="max-w-3xl">
            ...
            <p className="text-white/70 text-lg sm:text-xl max-w-xl mb-10 leading-relaxed font-normal">
      ...
      {/* ===== PARTNER / KLIEN YANG BEKERJA SAMA ===== */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-4">
            {partners.map((partner) => (
              <div
                key={partner.name}
                className="flex flex-col items-center justify-center p-4 rounded-2xl border border-[#E2E8F0] hover:border-primary/30 hover:shadow-md transition-all group bg-cream/40"
              >
                <div className="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center mb-2 group-hover:bg-primary/20 transition-colors">
                  <span className="text-primary font-bold text-xs text-center leading-tight whitespace-pre-line">{partner.abbr}</span>
                </div>
      ...
      <section className="py-24 bg-white" id="services">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {services.map((service, i) => (
              <Link
                key={service.title}
                href={service.href}
                className={`group relative p-7 rounded-2xl border transition-all duration-200 hover:-translate-y-1 ${i === 0
          ...
          <div className="text-center mt-10">
            <Link
              href="/services"
              className="inline-block border-2 border-dark text-dark hover:bg-dark hover:text-white px-7 py-3.5 rounded-xl font-bold text-sm transition-all"
            >
              Lihat Semua Layanan
            </Link>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <div
                key={t.name}
                className={`p-7 rounded-2xl border transition-all ${i === 1
                    ? "bg-dark text-white border-dark"
                    : "bg-cream border-cream hover:border-[#E2E8F0]"
                  }`}
              >
                ...
                <p className={`text-sm leading-relaxed mb-7 ${i === 1 ? "text-white/70" : "text-dark/65"}`}>
                  "{t.text}"
                </p>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center font-bold text-xs flex-shrink-0">
                    {t.avatar}
                  </div>
                  <div>
